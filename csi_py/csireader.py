...
class CSIReader:
    def __init__(self, port, amplitude_callback=None, phase_callback=None, raw_callback=None, rate=None, detect_static_samples=10):
        self.port = port
        self.csi_data_complex = np.zeros([CSI_DATA_INDEX, CSI_DATA_COLUMNS], dtype=np.complex64)
        self.amplitude_callback = amplitude_callback if amplitude_callback is not None else self.default_callback
        self.phase_callback = phase_callback if phase_callback is not None else self.default_callback
        self.raw_callback = raw_callback if raw_callback is not None else self.default_callback
        self._stop = False
        ...
        try:
            while not self._stop:
                ...
                if not self.rate or (now - last_process) >= interval:
                    ...
                    self.raw_callback(filtered_raw)
                    last_process = now
        finally:
            ser.close()